# OSF Pre-Registration (v1 draft): Alpha-M + Gamma + Delta + Combined-Alpha-Gamma

**Version:** v1 draft (internal, pre-deposit)
**Drafted:** 2026-04-19
...
- **Implementation:** PyMC with bridge sampling (Gronau et al. 2017) primary; R brms + BayesFactor as fallback if PyMC validation vs R ground truth fails on synthetic data (stats-pipeline task-006 will validate in Sub 1.2 Wave 2).
...
| ρ_true | Original (14×10, no attenuation) | Recalculated (14 S2 × ~7 eff. crossed, 20% attenuation) | Recalculated (9 MLFF × 10, 20% attenuation) |
|--------|-----------------------------------|---------------------------------------------------------|---------------------------------------------|
| 0.3 | ~20% | [v1 placeholder — Python simulation will populate in Appendix B v2] | [v1 placeholder] |
| 0.5 | ~42% | ~58-66% (projection based on IP §1 "~74% at 14×10 uncorrected" scaled by 0.78-0.80 attenuation factor) | ~48-56% |
| 0.7 | ~75% | ~82-88% | ~68-76% |
...
**Interpretation:** At ρ_true = 0.5, the post-expansion combined-paper design retains power > 55% after the 20% ICC attenuation correction. This is below the conventional 80% threshold but acceptable for a Bayesian-evidence framing where BF quantification is the primary result, not frequentist rejection. A BF₁₀ in the 1-3 range under JZS is pre-registered as "anecdotal / inconclusive"; the decision rule proceeds to separation under S3 if BF₁₀ projected < 1 at the D4 pilot analysis.

**Note:** Final power-analysis numbers will be refined in the v2 amendment after task-006 stats-pipeline synthetic-data validation in Sub 1.2 Wave 2. [v1 placeholder, will refine in v2 amendment]
...
**[v1 placeholder, will refine at deposit time: coordinate with head-1.2 to record the commit hash pinned to the OSF deposit; target 2026-05-14 locked commit]**

All code (pre-reg version) committed and tagged `osf-prereg-v1` at time of deposit.
...
Implementation: PyMC + bridge sampling (Gronau et al. 2017) primary; R brms + BayesFactor fallback. The Sub 1.2 task-006 stats-pipeline agent validates PyMC vs R output on synthetic data; if mismatch, R is adopted and documented in the stats-pipeline README.

---

## Appendix B: Power Analysis Code

The Python script below runs a parametric simulation for the combined-paper design under the hierarchical model in Appendix A. It reports power (P(BF₁₀ > 3 | ρ_true)) for ρ_true ∈ {0.3, 0.5, 0.7} and ICC attenuation ∈ {0.15, 0.20, 0.25}.

```python
"""
Appendix B: Pre-registered power analysis for the Combined paper.
Design: 14 S2-counted proteins × 10 generators crossed for integration
        (9 MLFF-feasible × 10 generators for the MLFF sub-benchmark).
ICC convergence attenuation: primary = 0.20; sensitivity = 0.15, 0.25.
Prior: JZS Cauchy(0, 1) on standardized slope.
Decision rule: BF_10 > 3 under JZS.
n_sim per (rho, att) cell: 10,000 draws.
Random seed: 20260515 (locked to OSF deposit target date).

Requires: pymc >=5.10, arviz, scipy, numpy.
Expected wall time on 1 CPU core: ~12-18 hours per (rho, att) cell.
Parallelization: trivially embarrassing; use 1 core per (rho, att) cell.
"""
...
N_PROTEINS = 14          # S2-counted
N_GENERATORS = 10
N_SIM = 10_000
RHO_GRID = [0.3, 0.5, 0.7]
ATT_GRID = [0.15, 0.20, 0.25]
ICC_RAW = 0.7            # IP §1 conservative estimate
SIGMA_PROTEIN = 0.08     # Phase 0 pilot calibration
SIGMA_GENERATOR = 0.10   # Phase 0 pilot calibration
SIGMA_NOISE = 0.12       # Residual SD after ICC correction

def simulate_dataset(rho, att, rng):
    """Generate one synthetic dataset under the Appendix A hierarchical model."""
    icc_eff = ICC_RAW * (1 - att)
    u = rng.normal(0, SIGMA_PROTEIN, N_PROTEINS)
    v = rng.normal(0, SIGMA_GENERATOR, N_GENERATORS)
    x = rng.standard_normal((N_PROTEINS, N_GENERATORS))           # Gamma Spearman
    beta1 = rho                                                    # standardized
    y = beta1 * x + u[:, None] + v[None, :] + rng.normal(0, SIGMA_NOISE, (N_PROTEINS, N_GENERATORS))
    return x.flatten(), y.flatten(), np.repeat(np.arange(N_PROTEINS), N_GENERATORS), np.tile(np.arange(N_GENERATORS), N_PROTEINS)

def bf10_jzs(x, y, p_idx, g_idx):
    """JZS Bayes factor via bridge sampling on the Appendix A model."""
    with pm.Model() as m_h1:
        sp = pm.HalfCauchy("sp", 0.5)
        sg = pm.HalfCauchy("sg", 0.5)
        s = pm.HalfCauchy("sigma", 1.0)
        b0 = pm.Normal("b0", 0, 10)
        b1 = pm.Cauchy("b1", 0, 1)                                 # JZS prior
        u = pm.Normal("u", 0, sp, shape=N_PROTEINS)
        v = pm.Normal("v", 0, sg, shape=N_GENERATORS)
        mu = b0 + b1 * x + u[p_idx] + v[g_idx]
        pm.Normal("obs", mu=mu, sigma=s, observed=y)
        idata_h1 = pm.sample(2000, tune=1000, progressbar=False, random_seed=20260515)
        lml_h1 = pm.sampling.jax.bridge_sampling(idata_h1)         # bridge sampler
    with pm.Model() as m_h0:
        sp = pm.HalfCauchy("sp", 0.5)
        sg = pm.HalfCauchy("sg", 0.5)
        s = pm.HalfCauchy("sigma", 1.0)
        b0 = pm.Normal("b0", 0, 10)
        u = pm.Normal("u", 0, sp, shape=N_PROTEINS)
        v = pm.Normal("v", 0, sg, shape=N_GENERATORS)
        mu = b0 + u[p_idx] + v[g_idx]                              # beta_1 = 0
        pm.Normal("obs", mu=mu, sigma=s, observed=y)
        idata_h0 = pm.sample(2000, tune=1000, progressbar=False, random_seed=20260515)
        lml_h0 = pm.sampling.jax.bridge_sampling(idata_h0)
    return np.exp(lml_h1 - lml_h0)

def power_cell(rho, att, n_sim=N_SIM, rng=RNG):
    """Fraction of simulated datasets yielding BF_10 > 3 under JZS."""
    wins = 0
    for _ in range(n_sim):
        x, y, p_idx, g_idx = simulate_dataset(rho, att, rng)
        bf = bf10_jzs(x, y, p_idx, g_idx)
        if bf > 3:
            wins += 1
    return wins / n_sim
...
if __name__ == "__main__":
    ...
    for rho in RHO_GRID:
        for att in ATT_GRID:
            p = power_cell(rho, att)
            table[(rho, att)] = p
            print(f"rho={rho}, att={att}: power={p:.3f}")
    # Save to phases/phase-1/subphase-1.2/output/scripts/stats/power_table.csv
```

**Pre-registered power table (populated by Sub 1.2 task-006 stats-pipeline validation). Primary row (att=0.20) is the planning baseline; rows for att=0.15 and att=0.25 are sensitivity reports.**

| ρ_true | att=0.15 | att=0.20 (primary) | att=0.25 |
|--------|----------|---------------------|----------|
| 0.3 | [v1 placeholder] | [v1 placeholder] | [v1 placeholder] |
| 0.5 | ~62% projected | ~58% projected | ~54% projected |
| 0.7 | ~86% projected | ~82% projected | ~78% projected |

**Projections above are order-of-magnitude estimates based on the 14×10 uncorrected ~74% power (IP §1) scaled by (1 - att). The final table will be populated in the v2 amendment by the Sub 1.2 task-006 synthetic-data validation. [v1 placeholder, will refine in v2 amendment]**
...
*End of pre-registration v1 draft. Word count (excluding appendices, references, and table frontmatter): approximately 4,100 words. Document length falls within the 3,000-6,000-word target range per `tasks/task-003-osf-prereg.md` §Verification. v2 refinement scheduled for 2026-05-13 after Sub 1.2 Wave 2 outputs (task-005 Delta baselines, task-006 stats-pipeline validation) are available.*
