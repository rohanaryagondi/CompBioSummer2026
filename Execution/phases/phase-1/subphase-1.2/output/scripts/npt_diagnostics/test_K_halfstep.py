#!/usr/bin/env python
"""
test_K_halfstep.py -- Round 3A: sentinel-bond + dt=0.5 fs.

Hypothesis: Round 2 test_J fixed the singleton-molecule problem (NVT 10 ps clean,
NPT survived to 12.5 ps before NaN, vs. 3.5 ps baseline). Residual NPT failure at
12.5 ps may stem from unconstrained protein C-H/N-H bonds (HBonds constraints
were stripped by openmm-ml's _removeBonds(allInSet=True)). At 1 fs timestep,
unconstrained C-H stretching (~3000 cm^-1, period ~10 fs) is at the edge of
integrator stability. Halving dt to 0.5 fs should mitigate this.

Configuration: identical to test_J except dt = 0.5 fs (was 1.0).
Step counts adjusted to fit gpu_devel 1-hour budget at the same effective
ps targets:
  - NVT 1 ps  (2000 steps at 0.5 fs)
  - NPT 13 ps (26000 steps at 0.5 fs) — must beat test_J's 12.5 ps to confirm fix

Total ~28000 steps × ~0.10 s/step ≈ 47 min. Plus ~5 min minimization.
"""
...
TEST_NAME = "K_halfstep"
DT_FS_K = 0.5                  # halved
EQUIL_NVT_PS = 1.0             # short NVT (we trust test_J showed 10 ps NVT clean)
NPT_TARGET_PS = 13.0           # must beat test_J 12.5 ps NPT failure
BAROSTAT_FREQ = 25
PROBE_LOG_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "logs", "test_K_probe.tsv"
)


def build_instrumented_hybrid(system, topology, positions, protein_atoms,
                              probe_log_handle):
    """Same as test_J's instrumented hybrid (no wrapping fix; probe each callback)."""
    import openmm
    ...
    log(f"Creating instrumented MACE-OFF24-medium hybrid for K-test "
    ...
    # f32 bypass with per-callback probe (mirrors test_J + test_I structure)
    ...
    def _f32_compute_probe(state, model, ptr, na, batch, pbc_t, idx, periodic,
        ...
        if _call_count[0] % 50 == 0 or _call_count[0] <= 20:
            try:
                ...
                span = pmax - pmin
                exc_x = max(0.0, max(pmax[0] - box_a, -pmin[0]))
                exc_y = max(0.0, max(pmax[1] - box_b, -pmin[1]))
                exc_z = max(0.0, max(pmax[2] - box_c, -pmin[2]))
                ...
                probe_log_handle.write(
                    f"{_call_count[0]}\t{box_a:.4f}\t{box_b:.4f}\t{box_c:.4f}\t"
                    f"{pmin[0]:.3f}\t{pmax[0]:.3f}\t{span[0]:.3f}\t{exc_x:.3f}\t"
                    f"{pmin[1]:.3f}\t{pmax[1]:.3f}\t{span[1]:.3f}\t{exc_y:.3f}\t"
                    f"{pmin[2]:.3f}\t{pmax[2]:.3f}\t{span[2]:.3f}\t{exc_z:.3f}\t"
    ...
    for _i in range(hybrid_system.getNumForces()):
        ...
        if isinstance(_force, openmm.PythonForce):
            ...
            log("  Using INSTRUMENTED float32 MACE (probe TSV per callback) for K-test")
            break
    ...
    # Probe header
    probe_log_handle.write(
        "call\tbox_a\tbox_b\tbox_c\t"
        "pmin_x\tpmax_x\tspan_x\texc_x\t"
        "pmin_y\tpmax_y\tspan_y\texc_y\t"
        "pmin_z\tpmax_z\tspan_z\texc_z\t"
...
def run_with_probe(simulation, label, target_ps, probe_log_handle, dt_fs):
    """Run integration steps with periodic NaN check + ps progress logging."""
...
def main():
    log(f"=== Test {TEST_NAME}: sentinel + dt={DT_FS_K} fs ===")
    log(f"  Hypothesis: dt=0.5 fs mitigates unconstrained C-H instability that")
    log(f"  caused test_J to crash at 12.5 ps NPT despite sentinel-bond fix")
    ...
    try:
        ...
        # Add MonteCarloBarostat (freq=0 initially)
        ...
        integrator = openmm.LangevinMiddleIntegrator(
            ...
            DT_FS_K * unit.femtosecond,
        )
        log(f"  Integrator timestep: {DT_FS_K} fs (HALVED from test_J's 1.0 fs)")
        ...
        if npt_result.get("failure_step"):
            log(f"  Failure step (NPT): {npt_result['failure_step']} "
                f"({npt_result['failure_step'] * DT_FS_K / 1000:.2f} ps)")
        log("=" * 65)
