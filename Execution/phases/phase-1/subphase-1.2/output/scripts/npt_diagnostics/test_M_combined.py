#!/usr/bin/env python
"""
test_M_combined.py -- Round 3C: sentinel + restore HBonds constraints (dt=1 fs).
...
def build_instrumented_hybrid(system, topology, positions, protein_atoms,
    ...
    log(f"Creating instrumented MACE-OFF24-medium hybrid for M-test "
    ...
    for _i in range(hybrid_system.getNumForces()):
        ...
        if isinstance(_force, openmm.PythonForce):
            ...
            log("  Using INSTRUMENTED float32 MACE for M-test")
            break
...
def main():
    ...
    try:
        ...
        # NEW: HBonds constraints (Round 3C)
