#!/usr/bin/env python
"""
test_P_extended.py -- Round 3D priority confirmation: sentinel + restore HBonds constraints (dt=1 fs).
...
def build_instrumented_hybrid(system, topology, positions, protein_atoms,
    ...
    log(f"Creating instrumented MACE-OFF24-medium hybrid for P-test "
    ...
    for _i in range(hybrid_system.getNumForces()):
        ...
        if isinstance(_force, openmm.PythonForce):
            ...
            log("  Using INSTRUMENTED float32 MACE for P-test")
            break
...
def main():
    ...
    try:
        ...
        # NEW: HBonds constraints (Round 3D priority confirmation)
