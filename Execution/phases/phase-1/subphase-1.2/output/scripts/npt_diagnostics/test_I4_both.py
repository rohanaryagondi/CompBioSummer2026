#!/usr/bin/env python
"""
test_I4_both.py -- MACE hybrid NPT with enforce_pbc + molecular wrap.

Combines the two fixes: pull a PBC-enforced state, then translate the whole
protein as a rigid blob into the primary cell.

Protocol: 10 ps NVT + 90 ps NPT.
Sub 1.2 task-009 NPT-patch attempt; Step 2 / mode='both'.
"""
...
def main() -> int:
    log(f"=== Test {TEST_NAME}: NPT with enforce_pbc + molecular wrap (both) ===")
    ...
    try:
        ...
        hybrid_system, _ = add_mace_hybrid_v2(
            ...
            use_f32=True, wrap_mode='both', simulation_holder=holder
        )
