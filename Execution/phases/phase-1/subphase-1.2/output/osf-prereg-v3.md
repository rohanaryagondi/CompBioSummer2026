# OSF Pre-Registration (v3): Alpha-M + Gamma + Delta + Combined-Alpha-Gamma

**Version:** v3 (deposit-ready; MACE NPT production recipe validated 2026-05-02 to 100 ps stable)
**Drafted:** 2026-04-19 (v1); 2026-04-21 (v2); 2026-05-01 (v3 initial); 2026-05-02 (v3 final after MACE NPT recipe validation)
...
### Changes from v2 → v3 (2026-05-02, head-1.2)

The v3 revision incorporates Sub 1.2 task-001 (MACE NPT diagnostics + remediation), task-002 (SO3LR vacuum), and the Round 3 NPT-fix iteration that yielded a working production recipe. All v3 changes are pre-deposit refinements of the v2 analysis plan; no hypothesis is added or removed, no decision rule is changed, no statistical test is re-specified. The four pre-registered priors (JZS, Skeptical, Informative, Flat) and the BF₁₀ > 3 / > 10 thresholds are unchanged.

1. **§2.1 Alpha-M H1 (stability):** Specifies NPT as the production ensemble (NVT remains a tested fallback). Sub 1.2 Round 3 demonstrated that openmm-ml's `_removeBonds(allInSet=True)` strips both protein bonds (causing OpenMM `findMolecules()` to treat each protein atom as a singleton — openmm-ml issue #91) AND inadvertently leaves protein C-H/N-H/O-H modes unconstrained. The combined "sentinel-bond + protein HBonds constraints + dt=1 fs" recipe (see §4) yields stable MACE NPT trajectories on solvated proteins (validated to 100 ps on WW; full 100,000-step trajectory passed at density 1.041 g/cm³, T 297.6 K, PE stable around -364,500 kJ/mol). See `shared/notes/1.2-mace-npt-fixed.md` for the full evidence chain.
2. **§4 Generator roster:** MACE-OFF24 row annotated with the production recipe (sentinel HarmonicBondForce with k=0 along the protein bonded graph + AMBER14 HBonds constraints re-added to the protein region after `createMixedSystem` + `MonteCarloBarostat` freq=25 + `LangevinMiddleIntegrator` dt=1 fs; reproducible from `phases/phase-1/subphase-1.2/output/scripts/npt_diagnostics/test_L_hbonds.py` and `test_P_extended.py`). SO3LR row annotated with vacuum NVT only + mandatory 500-ps Rg pre-screen + **mandatory neutral-protonation re-prep for net-charged proteins** (D/E protonated → ASH/GLH; K/R deprotonated → LYN); Sub 1.2 task-002 v1 demonstrated 2/5 stable at pH 7 (GB1, UBQ); rescue with neutral-protonation cleared 500-ps Rg gates for GB3 + NTL9 (task-002 rescue 2026-05-02).
3. **§7.1 T1 status:** Updated with Sub 1.2 evidence — MACE-OFF24 NPT recipe validated to 100 ps stable on WW (Round 3D test_P, job 10441986); SO3LR vacuum 2/5 PASS (GB1, UBQ Tier B); D2 G1 (≥1 MLFF stable ≥10 ns on ≥3 Tier B proteins) is achievable in Sub 1.4 production via MACE NPT.
4. **§8 Trajectory Truncation Protocol:** No structural change. The MACE NPT trajectory is single-stage (NPT throughout production) — counts toward T_min from the post-equilibration window (typically first 100 ps discarded as MACE-NPT equilibration).
5. **§10 Power Analysis:** Retains the v2 9-protein MLFF-feasible projection as primary. Sensitivity tables B.3 (7×10) and B.4 (5×10) are reported as a pre-registered sensitivity bracket for tracked-amendment use; the UBQ MACE NPT non-generalization (2026-05-03) is now the load-bearing case for the 5×10 floor. Sub 1.4 production locks the actual MLFF-feasible-S2-counted count via tracked v4 amendment.
...
**H1 (primary, stability):** At least one machine-learning force field (MACE-OFF24 or SO3LR) produces NPT molecular-dynamics trajectories of at least 10 ns duration with no NaN/blow-up events on at least three Tier B proteins (out of {WW, GB3, GB1, NTL9, CI2, CspA, alpha-3D, Calbindin D9k, Ubiquitin}) out of the MLFF-feasible set. The pre-registered MACE-OFF24 production recipe (validated in Sub 1.2 Round 3, see §4 and `shared/notes/1.2-mace-npt-fixed.md`) addresses two openmm-ml bugs (issue #91 singleton-molecule scaling; the inadvertent stripping of protein H-bond constraints) via two complementary patches applied after `createMixedSystem`: (a) zero-strength sentinel HarmonicBondForce along the protein bonded graph; (b) re-added AMBER14 HBonds constraints for protein heavy-atom-to-H bonds. With these patches, MACE-OFF24 hybrid NPT runs stably at dt=1 fs with `MonteCarloBarostat` freq=25; the recipe was validated to 100 ps clean NPT on WW (test_P, job 10441986, density 1.041 g/cm³, T 297.6 K). SO3LR vacuum NVT is a complementary path (no solvent, no box; 2/5 stable proteins in Sub 1.2 task-002: GB1, UBQ). T1 (combined-paper GO) is met if H1 is met by either MLFF on ≥3 Tier B proteins. As a documented fallback if the MACE NPT recipe later fails on a specific protein, the two-stage classical-NPT-equilibration → MACE-NVT-production protocol (`output/npt_nvt_production_plan.md`) is also pre-registered.
...
|---|-----------|----------|---------------|---------------|
| 1 | MACE-OFF24 | MLFF (solvated NPT) | mace-torch + OpenMM-ML (H200 OpenCL hybrid, Option 5); production recipe = `MLPotential('mace-off24-medium').createMixedSystem` + `add_protein_sentinel_bonds` (zero-k HarmonicBondForce along protein bonded graph, fixes findMolecules singleton scaling per openmm-ml issue #91) + `add_protein_hbonds_constraints` (re-add AMBER14 H-bond rigid constraints stripped by `_removeBonds(allInSet=True)`) + `MonteCarloBarostat` 1 atm/300 K/freq=25 + `LangevinMiddleIntegrator` 300 K/1 ps⁻¹/dt=1 fs; validated 100 ps clean on WW (test_P 2026-05-02; final density 1.041 g/cm³, PE stable). NVT-after-classical-NPT-equil two-stage protocol (`output/npt_nvt_production_plan.md`) retained as fallback. | H200 |
| 2 | SO3LR | MLFF (vacuum NVT) | JAX-MD; vacuum NVT only (no explicit solvent / no box, no NPT phase needed); mandatory Rg pre-screen per protein (`shared/notes/1.2-so3lr-pilot-stability.md` §3.7); **mandatory neutral-protonation re-prep for net-charged proteins (\|Q\|>0)**: D/E carboxyl groups protonated (variants `ASH`/`GLH`); K/R amine/guanidinium deprotonated (`LYN` variant; ARG handled per case). Verified Sub 1.2 task-002 rescue: charge-neutralized GB3 + NTL9 prep cleared 500-ps Rg pre-screen with PE stable around -1413 to -1513 (vs v1 at-pH-7 prep silent structural explosion at ~100 ps). For net-neutral proteins (\|Q\|=0 at pH 7), standard pH-7 prep is sufficient (e.g., GB1, UBQ, WW). | RTX 5000 Ada |
...
**MLFF production protocol (locked v3, validated 2026-05-02 to 100 ps stable on WW).** MACE-OFF24 production runs in NPT directly with the Sub 1.2 Round 3 recipe: `MLPotential('mace-off24-medium').createMixedSystem(...)` → `add_protein_sentinel_bonds(...)` (zero-strength HarmonicBondForce along the protein bonded graph, fixes openmm-ml issue #91 singleton-molecule scaling) → `add_protein_hbonds_constraints(...)` (re-add AMBER14 H-bond rigid constraints stripped by `_removeBonds(allInSet=True)`) → `MonteCarloBarostat(1 atm, 300 K, freq=25)` → `LangevinMiddleIntegrator(300 K, 1 ps⁻¹, dt=1 fs)`. Both helpers live in `phases/phase-1/subphase-1.2/output/scripts/npt_diagnostics/npt_diag_common.py`; reproduction driver is `test_L_hbonds.py` (25 ps gpu_devel) and `test_P_extended.py` (100 ps priority queue confirmation). Without these patches MACE NPT NaNs within 5-25 ps for two independent reasons: (i) `findMolecules()` treats each protein atom as a singleton "molecule" and `MonteCarloBarostat::scaleCoordinates` scales each protein atom independently around the box origin, destroying bonded distances; (ii) the same bond stripping leaves protein C-H/N-H/O-H bonds unconstrained, and at dt=1 fs the C-H stretching mode (~3000 cm⁻¹) is at the edge of velocity-Verlet stability — barostat-driven box rescales then drive runaway integration. With both patches MACE NPT runs at the same throughput as Sub 1.1 NVT (~0.91 ns/day on RTX 5000 Ada, ~2.56 ns/day on H200 OpenCL hybrid). SO3LR is always run in vacuum NVT (no solvent, no box, no NPT phase needed); SO3LR runs require a mandatory 500-ps Rg-monitoring pre-screen per protein (Sub 1.2 task-002 demonstrated GB3 and NTL9 silent structural explosion at ~100 ps in vacuum despite normal logged T/E; proteins with Rg ratio > 1.5 at 500 ps are excluded from SO3LR production). The two-stage NVT fallback (classical NPT equilibration → MACE NVT production, `output/npt_nvt_production_plan.md`) is retained as a documented fallback if reviewers later request a strict-NVT comparison or if MACE NPT fails on a specific protein in Sub 1.4 production.
...
|----|-----------|-------------|----------------------|
| T1 | ≥1 MLFF stable ≥10 ns on ≥3 Tier-B proteins (MACE NPT recipe per §4 OR SO3LR vacuum NVT) | Phase 1 pilot (Sub 1.1 D1 PASS; Sub 1.2 task-001 + task-002 5 ns evidence + Round 3 NPT-recipe validation to 100 ps; Sub 1.4 10 ns production) | ON TRACK. **Sub 1.1 evidence:** MACE-OFF24 NVT 1 ns Crambin D1 PASS. **Sub 1.2 evidence:** (a) MACE NPT initially NaN'd within 5-25 ps under naïve setup (Round 1 Tests A=classical PASS / B/F/v6/H=MACE hybrid FAIL); root cause identified as openmm-ml issue #91 (singleton-molecule scaling) plus inadvertent stripping of protein H-bond constraints; **Round 3 fix recipe (sentinel-bond + protein HBonds constraints + dt=1 fs) validated to 100 ps clean on WW** (test_P, job 10441986/p3w7m9kc, density 1.041 g/cm³, T 297.6 K, PE stable at -364,840 kJ/mol); 25 ps clean on GB3 (probe TIMEOUT 10458154/b8r3kt5x; recipe generalizes to GB3 system size); recipe documented in §4 and `shared/notes/1.2-mace-npt-fixed.md`. **UBQ MACE NPT non-generalization (2026-05-03)**: dt-sweep across {1.0, 0.5, 0.25} fs all NaN at 7-9.6 ps, pattern asymptotic — pathology architectural for UBQ-class system size (~17K solvated atoms); UBQ MACE production pivots to NVT-only fallback (`output/npt_nvt_production_plan.md`) per documented R1 closure-master-plan fallback. (b) SO3LR vacuum NVT 5 ns: 2/5 proteins PASS (GB1 Tier B, UBQ Tier B both stable; WW Tier A NaN at 0.7 ns; GB3 Tier A silent structural explosion at ~100 ps; NTL9 Tier B explosion at ~100 ps + NaN at 4.4 ns); **rescue via neutral-protonation re-prep (per §4) cleared 500-ps Rg pre-screen for GB3 + NTL9** (task-002 rescue gates 2026-05-02); 5 ns full rescues in flight. T1 satisfaction in Sub 1.4 requires ≥3 Tier B passes across MACE NPT + SO3LR (planned: WW NPT, GB3 NPT, GB1 NPT, NTL9 NPT, **UBQ NVT** (per architectural fallback)); SO3LR vacuum after rescue contributes 5 Tier A/B passes if 5/5 succeed. |
...
1. For each protein p in the active benchmark, let T_min(p) = min over generators g of the stable-trajectory duration that generator g produced for protein p after stability filtering (NaN-free, T and P within tolerance, density physical for solvated runs, Rg ratio < 2.0 vs initial frame).
...
**MLFF generators — equilibration-window accounting (v3 clarification).** All trajectory generators discard a 50-100 ps equilibration window before counting frames toward T_min(p). Specifically:

- MACE-OFF24 (NPT, recipe per §4): the first 100 ps of MACE NPT is discarded as equilibration (allows the box to converge to its MACE-equilibrated density and the protein to relax under MACE forces). The remainder of the trajectory counts toward T_min for MACE-OFF24. If a Sub-1.4 production run later uses the two-stage classical-NPT-equil → MACE-NVT-prod fallback (`output/npt_nvt_production_plan.md`), only the MACE NVT production phase counts toward T_min (Stage 1 classical forces are not MACE).
- SO3LR vacuum NVT: the first 50 ps post-relaxation (from `--relax`) is excluded as equilibration, and the remaining stable trajectory is counted toward T_min for SO3LR.
- Classical generators (AMBER, CHARMM, a99SB-disp): the first 50 ps of NVT run is excluded as equilibration and the remainder counts toward T_min.
- Generative generators (BioEmu, Boltz-2, AlphaFlow, Garnet) produce ensembles, not trajectories; T_min does not apply, but their physical-conformation count after stability filtering is reported alongside T_min for the trajectory generators.
...
**v3 sensitivity: MLFF-feasible set updated post-Sub-1.2 closure.** The 9-protein MLFF-feasible projection in v2 was made before Sub 1.2 stability data. Sub 1.2 closure evidence: (a) MACE NPT recipe validated to 100 ps clean on WW + 25 ps clean on GB3 (Round 3 fix per §4). **UBQ MACE NPT does NOT generalize** — dt-sweep across {1.0, 0.5, 0.25} fs all NaN at 7-9.6 ps; pathology architectural for ~17K-atom solvated UBQ system; UBQ pivots to NVT-only via `output/npt_nvt_production_plan.md`. The Sub 1.4 MACE NPT planned set is now {WW, GB3, GB1, NTL9} (Tier B; recipe expected to generalize to systems ≤ ~10K atoms based on WW+GB3 pass) plus UBQ NVT. (b) SO3LR vacuum 2/5 PASS in v1; rescue via neutral-protonation re-prep (per §4) cleared 500-ps Rg gates for GB3 + NTL9 (Tier A/B); 5 ns full rescues in flight (target 5/5 PASS). The combined MACE+SO3LR feasible-and-S2-counted set is plausibly close to the v2 projection of 9 proteins (NPT for 4 Tier B + NVT for UBQ + SO3LR coverage); sensitivity tables at 7×10 (Table B.3) and 5×10 (Table B.4) — see Appendix B — are pre-registered as a tracked-amendment safeguard if Sub 1.4 production reveals per-protein MACE NPT instabilities at production scale (10 ns) beyond what was seen at probe scale (50-100 ps). At the ρ_true = 0.5 reference: 9 MLFF (primary, n_eff=14.9) → 39% power; 7 MLFF (sensitivity, n_eff=11.6) → 30%; 5 MLFF (floor, n_eff=8.3) → 17%. **If Sub 1.4 production yields fewer than 7 MLFF-feasible-S2-counted proteins, separation S3 is more likely to fire at D4** (BF projected < 1 under JZS); the combined-paper claim then rests on the 14-protein S2-counted design (n_eff=16.9, 45% power at ρ=0.5) without the MLFF sub-benchmark. The actual MLFF-feasible-S2-counted count is locked at the end of Sub 1.4 and any deviation from the v3 projection becomes a tracked OSF amendment.
...
All code (pre-reg version) will be committed and tagged `osf-prereg-v3` at time of deposit.
...
- Power-analysis refinements after task-006 validation (RESOLVED in v3 deposit; computed values populated in Appendix B from validated stats pipeline)
- MLFF production-tier selection (scheduled: v4 amendment after Sub 1.4 planning; will lock per-protein NPT/NVT decisions and final MLFF-feasible-S2-counted count)
- **(NEW v3 → v4 amendment trigger)** SO3LR rescue methodology validation: if Sub 1.2 task-002 rescue 5 ns full runs all PASS, the neutral-protonation re-prep (now in §4) is locked as the production protocol for net-charged proteins. If any rescue fails at 5 ns, an amendment will document the failure mode and revised SO3LR scope.
...
**Table B.3: 7 MLFF-feasible proteins x 10 generators (v3 sensitivity, n_sim=2,000 per cell)**

| ρ_true | att=0.15 (n_eff=11.0) | att=0.20 (primary, n_eff=11.6) | att=0.25 (n_eff=12.2) |
|--------|-----------------------|---------------------------------|-----------------------|
| 0.3 | 9% | 10% | 10% |
| 0.5 | 27% | 30% | 30% |
| 0.7 | 61% | 68% | 67% |

**Table B.4: 5 MLFF-feasible proteins x 10 generators (v3 sensitivity-floor, n_sim=2,000 per cell)**

| ρ_true | att=0.15 (n_eff=7.9) | att=0.20 (primary, n_eff=8.3) | att=0.25 (n_eff=8.7) |
|--------|----------------------|--------------------------------|----------------------|
| 0.3 | 7% | 7% | 8% |
| 0.5 | 18% | 17% | 21% |
| 0.7 | 41% | 41% | 50% |
...
| Attenuation | ICC_eff | n_eff (14x10) | n_eff (9x10) | n_eff (7x10) | n_eff (5x10) |
|-------------|---------|---------------|--------------|--------------|--------------|
| 0.15 | 0.595 | 16.0 | 14.2 | 11.0 | 7.9 |
| 0.20 | 0.560 | 16.9 | 14.9 | 11.6 | 8.3 |
| 0.25 | 0.525 | 17.9 | 15.7 | 12.2 | 8.7 |
...
*End of pre-registration v3. Changes from v2 → v3 (head-1.2, 2026-05-02 → 2026-05-03): (1) §4 MACE-OFF24 row annotated with Round 3 production recipe (sentinel-bond + HBonds constraints + dt=1 fs + MCB freq=25); validated to 100 ps clean on WW + 25 ps clean on GB3. (2) §4 SO3LR row commits to mandatory neutral-protonation re-prep methodology for net-charged proteins (D/E protonated → ASH/GLH; K/R deprotonated → LYN); validated by Sub 1.2 task-002 rescue gates (GB3 + NTL9 cleared 500-ps Rg pre-screen). (3) §7.1 T1 row updated with multi-path D2 G1 evidence base post-UBQ-NPT-non-generalization (UBQ pivots to NVT). (4) §10 sensitivity narrative updated to reflect UBQ-specific NPT non-generalization (architectural for 17K-atom solvated systems). (5) §13 amendment provisions clarified (v4 amendment scheduled after Sub 1.4 planning; SO3LR rescue methodology lock contingent on 5 ns full-rescue PASS). Changes from v1 → v2 (preserved in v3): (i) power analysis tables populated with computed values from validated stats pipeline; (ii) Appendix B code replaced with corrected Fisher-z sampling approach; (iii) JZS BF implementation validated to <0.01% agreement with R.*
